...
int n = Yam_SurrenderPlayerItems.GetSize()
While(n > 0)
  n -= 1
...
akSpeaker.ShowGiftMenu(true, Yam_SurrenderPlayerItems, true)
...
Else
  ...
  int i = Yam_SurrenderPlayerItems.GetSize()
  Yam_SurrenderPlayerItems.RemoveAddedForm(Gold001)
  int req = ((0.2 + (0.2 * sur.isOutlaw as int)) * (i as float)) as int
  int collected = (pl.GetItemCount(Gold001) < numGold * 0.3) as int
  While(i > 0)
    i -= 1
    Form f = Yam_SurrenderPlayerItems.GetAt(i)
